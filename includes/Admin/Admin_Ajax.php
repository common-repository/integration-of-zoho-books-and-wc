...
class Admin_Ajax
{
    ...
    {
        add_action('wp_ajax_bit_wcb_generate_token', [$this, 'generateTokens']);
        add_action('wp_ajax_bit_wcb_get_log_data', [$this, 'getLogData']);
        add_action('wp_ajax_bit_wcb_get_integration_data', [$this, 'getIntegData']);
        add_action('wp_ajax_bit_wcb_add_integration_data', [$this, 'addIntegData']);
        add_action('wp_ajax_bit_wcb_save_integration_data', [$this, 'saveIntegData']);
        add_action('wp_ajax_bit_wcb_iwcmport_order_data', [$this, 'importOrderData']);
        add_action('wp_ajax_bit_wcb_refresh_organizations', [$this, 'refreshOrganizationsAjaxHelper']);
        add_action('wp_ajax_bit_wcb_refresh_fields', [$this, 'refreshFields']);
        add_action('wp_ajax_bit_wcb_zoho_books_get_warehouse', [$this, 'getWarehouse']);
        add_action('wp_ajax_bit_wcb_zoho_books_get_salespersons', [$this, 'getSalespersons']);
    }

    public function getLogData()
    {
        if (isset($_REQUEST['_ajax_nonce']) && wp_verify_nonce(sanitize_text_field($_REQUEST['_ajax_nonce']), 'bit_wc_zoho_books')) {
            $inputJSON = file_get_contents('php://input');
            $requestsParams = json_decode($inputJSON);
            $query = "SELECT * FROM {$this->wpdb->prefix}bit_wc_zoho_books_log ORDER BY {$requestsParams->sortBy->sortField} ";
            ...
            // get count
            $response['total_log'] = $this->wpdb->get_row("SELECT COUNT(id) as count FROM {$this->wpdb->prefix}bit_wc_zoho_books_log");
    ...
    {
        if (isset($_REQUEST['_ajax_nonce']) && wp_verify_nonce(sanitize_text_field($_REQUEST['_ajax_nonce']), 'bit_wc_zoho_books')) {
            $response['integ'] = $this->wpdb->get_results("SELECT * FROM {$this->wpdb->prefix}bit_wc_zoho_books_integration LIMIT 1");
    ...
    public function addIntegData()
    {
        if (isset($_REQUEST['_ajax_nonce']) && wp_verify_nonce(sanitize_text_field($_REQUEST['_ajax_nonce']), 'bit_wc_zoho_books')) {
            $inputJSON = file_get_contents('php://input');
            $requestsParams = json_decode($inputJSON);
            unset($requestsParams->wcBooksConf->newInteg);
            $integration_details = wp_json_encode($requestsParams->wcBooksConf);
            $result = $this->wpdb->query($this->wpdb->prepare("INSERT INTO {$this->wpdb->prefix}bit_wc_zoho_books_integration(integration_details) VALUE(%s)", $integration_details));
            if ($result) {
    ...
    public function saveIntegData()
    {
        if (isset($_REQUEST['_ajax_nonce']) && wp_verify_nonce(sanitize_text_field($_REQUEST['_ajax_nonce']), 'bit_wc_zoho_books')) {
            ...
            $idSql = "SELECT id FROM {$this->wpdb->prefix}bit_wc_zoho_books_integration ORDER BY id DESC LIMIT 1" ;
            $id = $this->wpdb->get_results($idSql)[0]->id;

            $result = $this->wpdb->query("UPDATE {$this->wpdb->prefix}bit_wc_zoho_books_integration SET integration_details = '$integration_details' WHERE id = $id ");
    ...
    public function importOrderData()
    {
        if (isset($_REQUEST['_ajax_nonce']) && wp_verify_nonce(sanitize_text_field($_REQUEST['_ajax_nonce']), 'bit_wc_zoho_books')) {
            ...
            wp_send_json_success('Orders Imported to Zoho Books Successfully', 200);
        }
    }
    ...
    public static function refreshOrganizationsAjaxHelper()
    {
        if (isset($_REQUEST['_ajax_nonce']) && wp_verify_nonce(sanitize_text_field($_REQUEST['_ajax_nonce']), 'bit_wc_zoho_books')) {
            ...
            $organizationsMetaApiEndpoint = "https://books.zoho.{$queryParams->dataCenter}/api/v3/organizations";
    ...
    public static function refreshFields()
    {
        if (isset($_REQUEST['_ajax_nonce']) && wp_verify_nonce(sanitize_text_field($_REQUEST['_ajax_nonce']), 'bit_wc_zoho_books')) {
            ...
            $customFieldsContactApiEndpoint = "https://books.zoho.{$queryParams->dataCenter}/api/v3/settings/fields?entity=contact&organization_id={$queryParams->orgId}";
            $authorizationHeader['Authorization'] = "Zoho-oauthtoken {$queryParams->tokenDetails->access_token}";
            ...
            if (isset($customFieldsContactApiResponse->fields) && count($customFieldsContactApiResponse->fields)) {
                foreach ($customFieldsContactApiResponse->fields as $cContactField) {
                    $booksCustomerFields[$cContactField->field_name_formatted] = [
            ...
            foreach ($booksCustomerFields as $key => $field) {
                if ($booksCustomerFields[$key]['required']) {
                    array_push($required, $booksCustomerFields[$key]['apiName']);
                }
            }
            uksort($booksCustomerFields, 'strnatcasecmp');
            $response['booksCustomerFields'] = [
                'fields'   => $booksCustomerFields,
                'required' => $required
            ];
            ...
            $customFieldsSalesOrderApiEndpoint = "https://books.zoho.{$queryParams->dataCenter}/api/v3/settings/fields?entity=salesorder&organization_id={$queryParams->orgId}";
            ...
            if (isset($customFieldsSalesOrderApiResponse->fields) && count($customFieldsSalesOrderApiResponse->fields)) {
                foreach ($customFieldsSalesOrderApiResponse->fields as $cSalesOrderField) {
                    $booksSalesFields[$cSalesOrderField->field_name_formatted] = [
            ...
            $required = [];
            foreach ($booksSalesFields as $key => $field) {
                if ($booksSalesFields[$key]['required']) {
                    array_push($required, $booksSalesFields[$key]['apiName']);
                }
            }
            uksort($booksSalesFields, 'strnatcasecmp');
            $response['booksSalesFields'] = [
                'fields'   => $booksSalesFields,
                'required' => $required
            ];
    ...
    public function getWarehouse()
    {
        if (isset($_REQUEST['_ajax_nonce']) && wp_verify_nonce(sanitize_text_field($_REQUEST['_ajax_nonce']), 'bit_wc_zoho_books')) {
            $inputJSON = file_get_contents('php://input');
            $queryParams = json_decode($inputJSON);
            $apiEndpoint = "https://books.zoho.{$queryParams->dataCenter}/api/v3/settings/warehouses?organization_id={$queryParams->orgId}";
            $requestHeaders['Authorization'] = "Zoho-oauthtoken {$queryParams->token}";
            $apiResponse = HttpHelper::get($apiEndpoint, null, $requestHeaders);
    ...
    public function getSalespersons()
    {
        if (isset($_REQUEST['_ajax_nonce']) && wp_verify_nonce(sanitize_text_field($_REQUEST['_ajax_nonce']), 'bit_wc_zoho_books')) {
            $inputJSON = file_get_contents('php://input');
            $queryParams = json_decode($inputJSON);
            $apiEndpoint = "https://books.zoho.{$queryParams->dataCenter}/api/v3/salespersons?organization_id={$queryParams->orgId}";
            $requestHeaders['Authorization'] = "Zoho-oauthtoken {$queryParams->token}";
            $apiResponse = HttpHelper::get($apiEndpoint, null, $requestHeaders);
            wp_send_json_success($apiResponse);
        }
